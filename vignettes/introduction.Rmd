...
the datasets on Penguins in Antartica, now in the R `datasets::penguins`.
The first step was to see how Penguin features, like body mass, flipper length and the length and depth of
their bills could be represented parametrically, and thus encoded in ways to permit drawing a penguin observation
using R graphics like `polygon()`, `segments()` and so forth.

provides a unique way to visualize multivariate penguin data by mapping measurements to visual features of schematic penguin drawings. This approach makes patterns in the data immediately apparent and intuitive.
