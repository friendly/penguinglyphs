...
draw_penguin_cartoon <- function(x, y,
  ...
  # Cartoon proportions (more exaggerated than realistic)
  body_width <- base_size * 1.2 * body_scale
  body_height <- base_size * 1.6 * body_scale
  head_size <- base_size * 0.5 * body_scale  # Smaller head (was 0.65)
  
  # Bill dimensions (larger and more prominent)
  bill_length <- head_size * 0.8 * bill_len_scale  # Longer bill (was 0.5)
  bill_depth <- head_size * 0.25 * bill_depth_scale  # Thicker bill
  
  # Flipper dimensions (more exaggerated)
  flipper_length <- body_height * 0.85 * flipper_scale  # Longer (was 0.6)
  flipper_width <- base_size * 0.3
  
  # Body position (compact oval)
  body_y <- y - head_size * 0.3
  ...
  # White belly (larger for cartoon style)
  belly_width <- body_width * 0.7
  belly_height <- body_height * 0.75
  belly_x <- x + belly_width * cos(theta)
  belly_y <- body_y + belly_height * sin(theta) * 0.9 - body_height * 0.1
  polygon(belly_x, belly_y, col = "white", border = NA)
  
  # Head position (higher up, smaller)
  head_y <- y + body_height * 0.6
  ...
  # Draw flippers (larger, more paddle-like)
  # Left flipper
  left_flipper_x <- c(
    x - body_width * 0.7,
    x - body_width * 0.9 - flipper_length * 0.3,
    x - body_width * 0.85 - flipper_length * 0.5,
    x - body_width * 0.6
  )
  left_flipper_y <- c(
    body_y + body_height * 0.3,
    body_y + body_height * 0.1,
    body_y - body_height * 0.4,
  ...
  # Draw feet (simple cartoon feet)
  feet_y <- body_y - body_height * 1.05
  feet_width <- body_width * 0.4
  
  # Left foot
  polygon(
    c(x - body_width * 0.3, x - body_width * 0.4, x - body_width * 0.2),
    c(feet_y, feet_y - base_size * 0.2, feet_y - base_size * 0.15),
    col = "orange", border = "black", lwd = 1.5
  )
